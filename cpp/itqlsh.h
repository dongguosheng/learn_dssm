...
class ITQLSH {
    public:
        ...
        virtual ~ITQLSH() {}
        inline void Train(const Eigen::MatrixXf &data_mat) {
            srand(time(NULL));
            int n_sample = int(data_mat.rows() * sample_rate);
            std::cout << "Sample Num: " << n_sample << std::endl;
            for(int i = 0; i < n_table; ++ i) {
                Eigen::MatrixXf sample_mat = GetSampleMat(data_mat, n_sample);
                std::cout << "Sample Done." << std::endl;
                pca(sample_mat);
                std::cout << "PCA Done." << std::endl;
                Eigen::MatrixXf r_mat(n_bit, n_bit);
                InitR(r_mat);
                std::cout << "Init R Done." << std::endl;
                Eigen::MatrixXf v_mat = sample_mat * pca_vec.back();
                Eigen::MatrixXf b_mat(n_sample, n_bit);
                for(int j = 0; j < n_iter; ++ j) {
                    std::cout << "n_table: " << i+1 << ", n_iter: " << j+1 << "(" << n_iter << ")\r" << std::flush;
                    b_mat = Sign(v_mat * r_mat);
                    Eigen::JacobiSVD<Eigen::MatrixXf> svd(b_mat.transpose() * v_mat, Eigen::ComputeThinU | Eigen::ComputeThinV);
        ...
        inline Eigen::MatrixXf GetSampleMat(const Eigen::MatrixXf &data_mat, int n_sample) {
            Eigen::MatrixXf sample_mat(n_sample, data_mat.cols());
        ...
        inline std::vector<Eigen::MatrixXf> Hash(const Eigen::MatrixXf &input) {
            ...
            for(size_t j = 0; j < pca_vec.size(); ++ j) {
                Eigen::MatrixXf b_mat = (input * pca_vec[j]) * r_vec[j];
        ...
        inline bool LoadText(const char *filename) {
            ...
            while(table_cnt < n_table) {
                Eigen::MatrixXf pca_tmp(n_dim, n_bit);
                int n_line = 0;
                while(n_line < n_dim) {
                    std::getline(fin, line);
                    float val;
            ...
            table_cnt = 0;
            while(table_cnt < n_table) {
                Eigen::MatrixXf r_tmp(n_bit, n_bit);
                int n_line = 0;
                while(n_line < n_bit) {
                    std::getline(fin, line);
                    float val;
    ...
    private:
        inline void pca(const Eigen::MatrixXf &sample_mat) {
            Eigen::MatrixXf centered = sample_mat.rowwise() - sample_mat.colwise().mean();
            Eigen::MatrixXf cov = (centered.transpose() * centered) / static_cast<float>(sample_mat.rows() - 1);
            Eigen::SelfAdjointEigenSolver<Eigen::MatrixXf> eig(cov);
            Eigen::MatrixXf pca_mat = eig.eigenvectors().rightCols(n_bit);
        ...
        float sample_rate;
        int n_iter;
        std::vector<Eigen::MatrixXf> pca_vec;
